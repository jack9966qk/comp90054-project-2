...
def getModSelf(tool,agent,features,gameState):
    ...
    if features["HasInvader1"]>0:
        dist1 = getInvaderDist(tool,tool.probMap[tool.team[0]][0],agent,tool.opp[0])
        dist2 = getInvaderDist(tool,tool.probMap[tool.team[1]][0],agent,tool.opp[0])
        if (dist1<dist2) and (agent.index==tool.team[0]):
            return "defense1"
    
    if features["HasInvader2"]>0:
        dist1 = getInvaderDist(tool,tool.probMap[tool.team[0]][0],agent,tool.opp[1])
        dist2 = getInvaderDist(tool,tool.probMap[tool.team[1]][0],agent,tool.opp[1])
        if (dist1<dist2) and (agent.index==tool.team[0]):
            return "defense2"
...
def getInvaderDist(tool,spos,agent,opp):
    allpos = tool.probMap[opp]
    myPos = spos
    minv = 999999
    
    for pos in allpos:
        if isInvader(tool,pos):
            minv = min(minv,agent.getMazeDistance(myPos, pos))
    return minv
